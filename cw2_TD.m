...
MDP = generateMap( roadBasisGridMaps, n_MiniMapBlocksPerMap, ...
    ...
    squaredError=zeros(episodeLength,1);
     ...
     alpha = max(1e-6, alpha*decayrate);
    for i = 1:episodeLength
        ...
        %based on action taken calculate Q_hat using W 
        Q_hat_old= sum ( sum( W(:,:,action_old) .* stateFeatures ) );
        ...
        for action = 1:3
            action_values(action) = ...
                sum ( sum( Q_test1(:,:,action) .* stateFeatures_new ) );
        end 
        % choose best action at next point in time  and calculate
        % corresponding qhat using W again
        [~, action_new] = max(action_values);
        Q_hat_new= sum ( sum( W(:,:,action_new) .* stateFeatures_new ) );
        ...
        oldW=W;
        W(:,:,action_old)=W(:,:,action_old)+Delta_W;
        
        %can plot several things to see convergence for example the max
        %differences between Weigths and see if this goes to 0
    ...
    %take mean from 24 steps for value of eacht episode
